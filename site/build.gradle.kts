...
//            description.set("Made with ❤ by Shevapro")
    ...
    <url>
        ...
        <lastmod>$currentTimestamp</lastmod>
                    ...
                    println("   📝 Blog posts: $postedBlogCount")
                    println("   💼 Portfolio items: $postedPortfolioCount")
                    println("   🔗 Total URLs: $totalUrls")
                    println("   🕐 Timestamp: $currentTimestamp")
                } else {
                    println("⚠️ Could not find closing </urlset> tag in existing sitemap")
                }
            } else {
                println("⚠️ Sitemap file does not exist at ${sitemapFile.absolutePath}")
            }
        }
    }
}
